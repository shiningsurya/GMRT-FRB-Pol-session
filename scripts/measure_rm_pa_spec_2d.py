...
def get_args ():
    ...
    add ( '-r', '--rmgrid', help='RM grid (min:max:steps)', default='-100:100:1024', dest='rmgrid' )
...
if __name__ == "__main__":
    ...
    times     = np.arange(I.shape[1])

    ## compute lambdas
    lam2      = np.power ( C / freq_list, 2 )
    # np.savez ("/tmp/ars/test_2d.npz", lam2=lam2, freq=freq_list, I=I, Q=Q, U=U, V=V, Ierr=I_err, Qerr=Q_err, Uerr=U_err, Verr=V_err)
    # adfd
    ...
    ################################
    """
    strategy for 2D (freq,time)
    inherent assumption we are fitting for one RM over time

    rm-synthesis ===> rms (time,) and pas (time,)

    from data(freq,time) subtract pas(time,) effect, 
    average in (time,)
    you get time averaged data(freq,)

    do bootstrapping to find RM and PA
    the RM value is the average RM 

    use the averaged RM to find the rmtf slope

    using data(freq,time) correct for averaged RM value
    measure PA(time,)
    ...
    ### {rm,pagrid}spec (rm_grid, time)
    rmspec, pagridspec  = paspec.rm_spectrum ( rm_grid ) 

    ### argmax ( time )
    ### peak_rms (time)
    ### peak_pas (time)
    armax     = np.argmax ( rmspec, axis=0 )
    peak_rms  = rm_grid [ np.argmax ( rmspec, axis=0 ) ]
    peak_pas  = np.array ( [pagridspec[am,im] for im,am in enumerate(armax)] )

    ### subtract peak_pas(time)
    ### and average to form PA(freq)
    paspec.subtract_patime ( peak_pas )

    ### fit 
    ### we would need to ignore pa_boot
    ### because we subtracted peak_pas(time)
    ### this pa is likely zero
    fitrm, rm_boot, pa_boot   = paspec.bootstrap_rmpa ( rm_grid, n_resamples=args.ntrials )

    ### we will measure PA using rm_boot
    ### data(freq,time) - rm_boot
    ### average and then measure PA(time)
    pa_time   = paspec.pa_time ( fitrm['rm'] )

    ### impulse RMTF
    ### using bootstrapped RM
    rmtf      = paspec.rmtf ( rm_grid, fitrm['rm'] )

    ### for plottings
    ### 2D (freq,time)
    padata      = 0.5 * np.angle ( paspec.qu_sub )
    padataerr   = 0.5 * np.angle ( paspec.qu_sub * np.exp ( -2.0j * ( fitrm['pa'] + fitrm['rm'] * paspec.w2d ) ) )
    ### 1D (freq)
    pafitted  = paspec.pa_sub
    pamodel   = 0.5 * np.angle ( np.exp ( 2.0j * ( fitrm['pa'] + fitrm['rm'] * paspec.mw2 ) ) )
    paerr     = 0.5 * np.angle ( np.exp ( 2.0j * ( paspec.pa_sub - fitrm['pa'] - fitrm['rm']*paspec.w2 ) ) )
    paerr     = np.ma.MaskedArray ( paerr, mask=paspec.fmask )

    ut    = "RM={rm:.3f}+-{rmse:.3f} PA0={pa:.3f}+-{pase:.3f}".format ( 
        rm = fitrm['rm_mean'], rmse = fitrm['rm_se'],
        pa = np.rad2deg(fitrm['pa_mean']), pase = np.rad2deg(fitrm['pa_se']),
    )
    ...
    axrr = fig.add_subplot ( gs[2,:], sharex=axpa )

    axrs  = fig.add_subplot ( gs[0, 0] )
    axtrm = fig.add_subplot ( gs[0, 1] )
    axtpa = fig.add_subplot ( gs[0, 2] )

    cc    = cycle ( ['red','green','blue','orange','cyan', 'magenta'] )

    for i in range(paspec.nsamp):
        axrs.plot ( rm_grid, rmspec[...,i], c=next(cc) )

    axrs.axvline ( fitrm['rm'], c='k', ls='-' )

    axtrm.scatter ( times, peak_rms, c='b', marker='.' )
    axtrm.axhline ( fitrm['rm'], c='k', ls='-' )

    axtpa.scatter ( times, np.rad2deg( peak_pas ), c='b', marker='.' )
    axtpa.plot ( times, np.rad2deg( pa_time ), c='k', )

    axpa.scatter ( paspec.w2, np.rad2deg(pafitted), c='k', marker='.' )
    axpa.plot ( paspec.mw2, np.rad2deg(pamodel), c='b',)

    axrr.scatter ( paspec.w2, np.rad2deg(paerr), c='k', marker='.' )
    axrr.axhline (0., ls=':', c='k', alpha=0.4 )

    axrs.set_title ('RM / rad m$^{-2}$')
    axrs.set_ylabel ('mag')
    # axtpa.set_xlabel ('Time / ms')
    # axtrm.set_xlabel ('Time / ms')
    axtpa.set_title ('PA / deg vs Sample')
    axtrm.set_title ('RM / rad m$^{-2}$ vs Sample')
    axpa.set_ylabel ('PA / deg')
    axrr.set_ylabel ('Error PA / deg')
    axrr.set_xlabel ('Wavelength$^{2}$ / m$^{2}$')
    ...
    # faxpa= axrr.secondary_xaxis ('top', functions=(to_freq, from_freq))

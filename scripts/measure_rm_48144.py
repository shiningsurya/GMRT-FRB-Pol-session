"""
for datasets of 48_144

which have been hacky calibrated using just a pulsar scan
"""
...
def get_args ():
    ...
    add ('-c','--cal', required=True, help='corresponding cal file', dest='cal')
...
if __name__ == "__main__":
    ...
    pkg       = np.load ( args.ar_file )
    ####################################
    freq_list = pkg['freqs'][...,0]
    Qdata     = pkg['Q'][...,0]
    Udata     = pkg['U'][...,0]
    I_err     = pkg['I_err'][...,0]
    Q_err     = pkg['Q_err'][...,0]
    U_err     = pkg['U_err'][...,0]
    lam2      = pkg['lam2'][...,0]
    data_freq_low    = freq_list.min()
    data_freq_high   = freq_list.max()
    ######################
    LLdata    = Qdata - 1.0j*Udata
    print (f"**************************")
    print (f" THERE IS A SIGN FLIP IN STOKES-U SOMEWHERE")
    print (f" I AM HAVING TO DO IT TO GET REASONABLE RESULTS")
    print (f" THIS IS NOT ME CHEATING, I AM LOOKING AT THE MAGNITUDE in RMSYNTHESIS")
    print (f" I DID THE SAME WHILE MEASURING XDELAY")
    print (f"**************************")
    calmodel  = np.exp ( -1.0j * ( ( cal['delay'] * np.pi * freq_list * 1E-3 ) + ( cal['bias_rad'] ) ) )
    LLcal     = LLdata * calmodel
    Q         = LLcal.real
    U         = LLcal.imag
    ######################
